...
from cralwer.items import NewHouseItem
...
class NbnewhouseSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        newHouseItem['capital_supervision_bank'] = items[1].xpath('text()').extract()[-1].strip()
        newHouseItem['capital_supervision_acount']= items[3].xpath('text()').extract()[-1].strip()
        newHouseItem['project_name'] = items[5].xpath('span/text()').extract()[0].strip()
        ...
        newHouseItem['licence_authority'] = items[13].xpath('text()').extract()[-1].strip()
        ...
        newHouseItem['online_saleable_area'] = pattern.findall(items[19].xpath('text()').extract()[-1])[0]
        newHouseItem['online_saleable_flats'] = pattern.findall(items[21].xpath('text()').extract()[-1])[0]
        newHouseItem['saleable_area'] = pattern.findall(items[23].xpath('text()').extract()[-1])[0]
        newHouseItem['saleable_flats'] = pattern.findall(items[25].xpath('text()').extract()[-1])[0]
        newHouseItem['sold_area'] = pattern.findall(items[27].xpath('text()').extract()[-1])[0]
        newHouseItem['sold_flats'] = pattern.findall(items[29].xpath('text()').extract()[-1])[0]
        newHouseItem['residential_area'] = pattern.findall(items[31].xpath('text()').extract()[-1])[0]
        newHouseItem['residential_flats'] = pattern.findall(items[33].xpath('text()').extract()[-1])[0]
        newHouseItem['sold_residential_area'] = pattern.findall(items[35].xpath('text()').extract()[-1])[0]
        newHouseItem['sold_residential_flats'] = pattern.findall(items[37].xpath('text()').extract()[-1])[0]
        newHouseItem['reserve_area'] = pattern.findall(items[39].xpath('text()').extract()[-1])[0]
        newHouseItem['reserve_flats'] = pattern.findall(items[41].xpath('text()').extract()[-1])[0]
        newHouseItem['saleable_parking_amount'] = pattern.findall(items[43].xpath('text()').extract()[-1])[0]
        newHouseItem['saleable_garage_amount'] = pattern.findall(items[45].xpath('text()').extract()[-1])[0]
        newHouseItem['sold_avg_price'] = pattern.findall(items[47].xpath('text()').extract()[-1].strip())[0]
